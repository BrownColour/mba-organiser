...
function renderTimeline(host, days){
  ...
  days.forEach(day=>{
    ...
    evs.forEach(ev=>{
      ...
      evBlocks += `
        <div class="tl-event ${cancelled?'cancelled':''}" data-id="${ev.id}" style="top:${top}px;height:${height}px;border-left-color:${color};background:${cancelled?'var(--surface-2)':'color-mix(in srgb, '+color+' 22%, var(--surface))'}">
          <div class="te-title">${escapeHtml(ev.title||(course?course.name:meta.label))}</div>
  ...
  cells.forEach(day=>{
    ...
    cellsHtml += `
      <div class="month-cell ${otherMonth?'other-month':''} ${isToday(day)?'today':''}" data-date="${toISO(day)}">
        ...
        ${shown.map(ev=>`<div class="cell-chip ${ev.status==='cancelled'?'cancelled':''}" data-id="${ev.id}" style="border-left-color:${colorForEvent(ev)}">${fmtTime(ev.startTime)?fmtTime(ev.startTime)+' · ':''}${escapeHtml(ev.title||TYPE_META[ev.type]?.label||'')}</div>`).join('')}
...
function agendaItemHtml(ev, showDate){
  ...
  return `
    <div class="agenda-item ${cancelled?'cancelled':''}" data-id="${ev.id}">
      <div class="agenda-time">${showDate?fmtDateShort(fromISO(ev.date))+' · ':''}${fmtTime(ev.startTime)}</div>
      <div class="agenda-bar" style="background:${colorForEvent(ev)}"></div>
