...
/* ============ Mutate helper: call API then refresh full state ============ */
async function mutate(fn, successMsg){
  try{
    await fn();
    const data = await API.getAll();
    STATE.trimesters = data.trimesters||[];
    STATE.courses = data.courses||[];
    STATE.events = data.events||[];
    STATE.attendance = data.attendance||[];
    if(!STATE.activeTrimesterId && STATE.trimesters.length){
      STATE.activeTrimesterId = STATE.trimesters[STATE.trimesters.length-1].id;
    }
    closeModal();
    renderCurrentView();
...
function submitTrimester(){
  ...
  mutate(async ()=>{
    const res = await API.addTrimester({name, startDate, endDate});
...
function submitCourse(id){
  ...
  if(id){
    mutate(()=>API.updateCourse({id, ...payload}), 'Course updated');
  } else {
    const tri = activeTrimester();
    if(!tri){ toast('Set up a trimester first', true); return; }
    payload.trimesterId = tri.id;
    mutate(()=>API.addCourse(payload), 'Course added');
  }
}
function submitDeleteCourse(id){
  ...
  mutate(()=>API.deleteCourse({id}), 'Course deleted');
}
...
function submitEvent(id){
  ...
  if(id){
    mutate(()=>API.updateEvent({id, ...base, date}), 'Event updated');
    return;
  }
  ...
  if(dowChips.length && repeatUntilEl){
    ...
    // walk day by day from anchor's week start to 'until', collecting matching weekdays on/after anchor date
    ...
    mutate(()=>API.addEventsBulk({items}), `Added ${items.length} sessions`);
  } else {
    mutate(()=>API.addEvent({...base, date}), 'Event added');
  }
}

function submitDeleteEvent(id){
  if(!confirm('Delete this event?')) return;
  mutate(()=>API.deleteEvent({id}), 'Event deleted');
}
...
function openRescheduleModal(id){
  ...
  openModal(`
    ...
    <div class="hint" style="margin-bottom:10px;">The original session will be marked <b>rescheduled</b> and a new event will be created and linked to it.</div>
    <div class="form-grid">
...
function submitReschedule(id){
  ...
  mutate(async ()=>{
    ...
    await API.updateEvent({ id: ev.id, ...ev, status:'rescheduled', linkedTo: created.id });
  }, 'Session rescheduled');
}
...
function openAttendanceModal(eventId){
  ...
  openModal(`
    ...
    <div class="hint" style="margin-bottom:10px;">${escapeHtml(ev.title || (courseById(ev.courseId)?.name||''))} · ${fmtDateShort(fromISO(ev.date))} · ${fmtTime(ev.startTime)}</div>
    <div class="form-field full">
      <div class="segmented" id="attSeg" style="width:100%;">
        <button type="button" data-v="present" class="${existing?.status==='present'?'active':''}">Present</button>
        <button type="button" data-v="absent" class="${existing?.status==='absent'?'active':''}">Absent</button>
        <button type="button" data-v="na" class="${(!existing||existing?.status==='na')?'active':''}">N/A</button>
      </div>
    </div>
...
function submitAttendance(eventId){
  const ev = STATE.events.find(e=>e.id===eventId);
  const status = document.querySelector('#attSeg button.active')?.dataset.v || 'na';
  const notes = document.getElementById('attNotes').value.trim();
  mutate(()=>API.setAttendance({ eventId, courseId: ev.courseId, date: ev.date, status, notes }), 'Attendance saved');
}
...
function renderAttendanceView(host){
  ...
  const summaryRows = STATE.courses.map(c=>{
    ...
    const marked = held.map(s=>attendanceForEvent(s.id)).filter(Boolean);
  ...
  html += summaryRows.length ? summaryRows.map(r=>`
    <div class="agenda-item" data-course="${r.c.id}" style="cursor:pointer;">
      ...
      <div class="agenda-body">
        ...
        <div class="agenda-meta">${r.total?`${r.present}/${r.total} present`:'No sessions marked yet'}</div>
      </div>
  ...
  if(!course){
    ...
    html += `<div class="card" style="margin-bottom:10px;font-weight:700;">${escapeHtml(course.name)}</div>`;
    html += `<div class="card">`;
    html += sessions.length ? sessions.map(s=>{
      const a = attendanceForEvent(s.id);
      const statusBadge = s.status==='cancelled' ? `<span class="badge badge-danger">Cancelled</span>`
        : a ? `<span class="badge ${a.status==='present'?'badge-success':a.status==='absent'?'badge-danger':''}">${a.status}</span>`
      ...
      return `<div class="agenda-item" data-mark="${s.id}" style="cursor:pointer;">
        ...
        <div class="agenda-body">
          <div class="agenda-title">${fmtTime(s.startTime)} ${s.room?'· Rm '+escapeHtml(s.room):''}</div>
        </div>
        ${statusBadge}
      </div>`;
